...
import { CredentialsService, Credentials } from '@core/services/credentials.service';
...
describe('CredentialsService', () => {
  ...
  describe('setCredentials', () => {
    it('should authenticate user if credentials are set', () => {
      // Act
      credentialsService.setCredentials({ access_token: '123', token_type: 'api' });

      // Assert
      expect(credentialsService.isAuthenticated()).toBe(true);
      expect((credentialsService.credentials as Credentials).access_token).toBe('123');
    });
    ...
    it('should persist credentials across sessions', () => {
      // Act
      credentialsService.setCredentials({ access_token: '123', token_type: 'api' }, true);
